...
class QuizPageState extends State<QuizPage> with WidgetsBindingObserver {
    ...
    if (_step.index < Step.NEXT.index)
      switch (i) {
        ...
        case 9:
          if (widget.readOnly ||
              !_isAnswered() ||
              (_isAnswered() && _question.newScoring != null))
            return Container(
        ...
        case 11:
          if (_question.exam != null && _visibleDashboad)
  ...
  void _readyQuiz() {
    if (_step == Step.INIT && _question != null) {
  ...
  Future<void> _next() async {
    if (_currentIndex < (widget.questions.length - 1)) {
      ...
      if (_question == null) _question = widget.questions[--_currentIndex];
      Navigator.pop(context);
      setState(() {
        _step = Step.INIT;
        _processing = false;
